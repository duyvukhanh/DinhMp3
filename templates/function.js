...
function enableReplay() {
    ...
    document.getElementById("shuffle-button").style.color = "lightgray";
    document.getElementById("shuffle-button").setAttribute("onclick", "enableShuffle()");
    isShuffle = false;
    isReplay = true;
}
...
function enableShuffle() {
    document.getElementById("shuffle-button").style.color = "crimson"
    document.getElementById("shuffle-button").removeAttribute("onclick");
    document.getElementById("shuffle-button").setAttribute("onclick", "disableShuffle()");
    document.getElementById("replay-button").style.color = "lightgray";
    document.getElementById("replay-button").setAttribute("onclick", "enableReplay()");
    isShuffle = true;
    isReplay = false;
...
function progessBar() {
    song.addEventListener('timeupdate', function () {
        ...
        if (isReplay) {
            ...
            nextSong = Math.floor(Math.random() * currentPlaylist.length); 
            if (per != 1) {
                let style = `background-image: -webkit-gradient(linear, 0% 0%, 100% 0%, color-stop(${per} , crimson), color-stop(${per} , #0A1A2A));border-radius : 20px;`;
                document.getElementById("track-control").removeAttribute("style");
                document.getElementById("track-control").setAttribute("style", style);
                ...
                document.getElementById("time-start").innerHTML = `00:00`;
                document.getElementById("time-stop").innerHTML = timeConvert(song.duration);
                playFirstTime(currentPlaylist[nextSong].id);
                song.play();
            }
        } else {
            if (per != 1) {
